...
def get_county_attributes(svg_file_name):
    ...
    """
    dom1 = mnd.parse(svg_file_name)  # parseString also exists
    id_list = [
        path.getAttribute('id')
        for path in dom1.getElementsByTagName('path')
    ]
    d_list = [
        path.getAttribute('d')
        for path in dom1.getElementsByTagName('path')
    ...
    dom1.unlink()
    county_attribute_list = list(zip(id_list, d_list))
...
def get_boundary_coordinates(boundary_data):
    ...
    boundary_data = boundary_data.split('L')
    boundary_data = [data for data in boundary_data if data != '']
    boundary_coordinates = list()
    for data_string in boundary_data:
        (x, y) = data_string.split(',')
        boundary_coordinates.append((float(x), float(y)))
...
def process_county_attributes(svg_file_name, csv_file_name):
    ...
    with open(csv_file_name, 'w', newline='') as f:
        ...
        for (id, boundary_data) in county_attribute_list:
            boundary_coordinates = get_boundary_coordinates(boundary_data)
            country_center = compute_county_center(boundary_coordinates)
            writer.writerow([id, country_center[0], country_center[1]])
