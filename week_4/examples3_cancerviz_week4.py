...
def create_riskmap(colormap):
    ...
    risk_norm = clr.Normalize(vmin=MIN_LOG_RISK, vmax=MAX_LOG_RISK)
    color_mapper = cm.ScalarMappable(norm=risk_norm, cmap=colormap)
...
def draw_cancer_risk_map(joined_csv_name, map_name, num_counties=None):
    ...
    risk_map = create_riskmap(cm.Accent)    # choose between Accent, hsv, and jet
